...
export default function StatusIcon({ status }: { status: TaskStatus }) {
  if (status === 'pending') {
    return <span className="w-4 h-4 rounded-full border-2 border-[var(--color-text-muted)]" />;
  }
  if (status === 'converting') {
    return (
      <svg className="w-4 h-4 animate-spin-icon text-[var(--color-accent)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
